...
public class ResponseTest02 extends HttpServlet {
	...
	/*
	 	- redirect
	 		...
	 		=> 이 이동방식은 무조건 GET방식으로 요청하기 때문에 URL의 길이에 제한이 없다.
	
			=> 호출하는 곳에서 사용하던 request, response를 유지하지 못하고	새로운 request와 response가 매개변수에 전달된다.
	*/
		...
		//형식) sendRedirect("전체 URL주소");
//		response.sendRedirect(
//				"/servletTest/ResponseRedirectTest.do");
		
		//새로운 페이지에 데이터를 넘겨주려면 URL 주소 뒤에 물음표(?)를 붙이고
